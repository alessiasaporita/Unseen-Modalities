...
class EPICKitchensTrain(Dataset):
    def __init__(
        ...
        if self.modality=='rgb':
            with open("epic-annotations/EPIC_100_train.csv") as f:
                ...
                for i, row in enumerate(f_csv):
                    ...
                    if row[0] not in self.sample_dict:
                        continue
                    if 'RGB' not in self.sample_dict[row[0]]:
                        continue
                    samples.append(row)
            f.close()
            ...
            with open("epic-annotations/EPIC_100_train.csv") as f:
                ...
                for i, row in enumerate(f_csv):
                    ...
                    if row[0] not in self.sample_dict:
                        continue
                    if 'Audio' not in self.sample_dict[row[0]]:
                        continue
                    samples.append(row)
            f.close()
            with open("epic-annotations/EPIC_100_validation.csv") as f:
        ...
        self.data = samples #62297 samples: 31213 RGB,  31084 Audio
    ...
    def __getitem__(self, index):
        ...
        if self.modality=="rgb":
            ...
            output_data["RGB"] = torch.rand(3, 32, 224, 224) #(3, 32, 224, 224)
